...
func _input(event: InputEvent) -> void:
	if event.is_action_pressed(GameConstants.INPUT_INTERACT) and can_interact and current_interactions.size() > 0:
		can_interact = false
		interact_label.hide()
		
		var target = current_interactions[0]
		if target and not target.auto_start:
			if target.has_method("action"):
				await target.action()
		
		if target and target.interact is Callable:
